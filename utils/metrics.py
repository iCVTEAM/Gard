...
class R1_mAP():
    ...
    def compute_acc(self):  # called after each epoch
        valnum=len(self.pids)
        assert valnum > 0, "VALNUM sum is zero ,stop now"
        print("start evaluate num: ")
        print(valnum)

        acc=0.0
        for i in range(valnum):
            pred=torch.argmax(self.feats[i])
            pred=pred.cpu().numpy()
            if pred==self.pids[i]:
                acc=acc+1.0
