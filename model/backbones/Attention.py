...
class ATT():
    ...
    def calc_mask_iou(self,pred_mask,gt_mask):
        pred_mask = pred_mask.clone().detach()
        ...
        inter = (pred_mask * gt_mask).sum(dim=(0,1))
        union = (pred_mask + gt_mask).sum(dim=(0, 1))

        miou = inter/(union+0.01)
    ...
    def NMS_crop(self, attention_maps, input_image):
        ...
        for i in range(batch_size):
            ...
            for enum in range(num_topk):
                ...
                '''
                one_vec =torch.ones_like(mask[enum])
                mask[enum] = torch.where(mask[enum] >= mask[enum].max() * threshold,mask[enum],one_vec)
                '''
                ...
                if use_segmentation_iou == True:
                    for vis_i in vis:
                        iou = self.calc_mask_iou(mask[enum],vis_i.squeeze())
                        if iou > 0.2:
                            flag_use = 0
                            break
                        print(iou)
                ...
                if flag_use == 1:
                    ...
                    out_img = torch.nn.functional.interpolate(out_img, size=(W, H), mode='bilinear',
                                                              align_corners=True)
                    newmask = torch.zeros_like(mask[enum])
                    newmask[height_min:height_max, width_min:width_max] = 255
                    vis.append(mask[enum].unsqueeze(0))
                    #vis.append(newmask.unsqueeze(0))
    ...
    def feature_crop(self, attention_maps, input_image):
        ...
        for i in range(batch_size):
            ...
            for enum in range(num_select):
                ...
                # filtering the background features
